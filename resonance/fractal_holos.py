...
class FractalHOLOS:
    ...
    def memory_usage_mb(self) -> float:
        ...
        try:
            ...
            # Fallback to estimation if psutil not available
            # But multiply by 6 to be closer to reality!
            frontier_mb = len(self.frontier) * 300 / (1024 * 1024)
            solved_mb = len(self.solved) * 60 / (1024 * 1024)
            children_mb = len(self.children) * 150 / (1024 * 1024)
            children_mb += sum(len(c) for c in self.children.values()) * 8 / (1024 * 1024)
            parents_mb = len(self.parents) * 150 / (1024 * 1024)
            parents_mb += sum(len(p) for p in self.parents.values()) * 8 / (1024 * 1024)
            all_seen_mb = len(self.all_seen) * 60 / (1024 * 1024)
            
            total = frontier_mb + solved_mb + children_mb + parents_mb + all_seen_mb
            return total * 1.5  # Safety margin
    
    def checkpoint_progress(self):
        """Save current solving progress (mid-region checkpoint) - VERSIONED"""
        # Create versioned checkpoint (don't overwrite!)
        ...
        version_path = os.path.join(self.save_dir, f'checkpoint_d{self.depth}_{timestamp}.pkl')
        with open(version_path, 'wb') as f:
            pickle.dump(checkpoint, f)
        
        # Also save as "latest" for easy resume
        latest_path = os.path.join(self.save_dir, 'checkpoint_latest.pkl')
        with open(latest_path, 'wb') as f:
            pickle.dump(checkpoint, f)
        
        size_mb = os.path.getsize(version_path) / (1024 * 1024)
        print(f"  [Checkpoint saved: depth={self.depth}, solved={len(self.solved):,}, size={size_mb:.1f}MB]")
    ...
    def load_checkpoint(self) -> bool:
        ...
        if not os.path.exists(latest_path):
            ...
            checkpoints = [f for f in os.listdir(self.save_dir) if f.startswith('checkpoint_d')]
            if not checkpoints:
                return False
            # Use most recent
            checkpoints.sort()
            latest_path = os.path.join(self.save_dir, checkpoints[-1])
        ...
        try:
            ...
            self.parents = defaultdict(list, checkpoint.get('parents', {}))
            ...
            print(f"  All seen: {len(self.all_seen):,}, Children tracked: {len(self.children):,}")
            return True
    ...
    def clear_checkpoint(self):
        """Archive checkpoint after region complete (don't delete!)"""
        latest_path = os.path.join(self.save_dir, 'checkpoint_latest.pkl')
        if os.path.exists(latest_path):
            # Move to archive, don't delete
            archive_path = os.path.join(self.save_dir, f'checkpoint_archived_{int(time.time())}.pkl')
            os.rename(latest_path, archive_path)
...
def main():
    ...
    if args.resume:
        if solver.load_checkpoint():
            ...
            print(f"  Region: {solver.current_region_name}")
            print(f"  Depth: {solver.depth}")
            print(f"  Solved: {len(solver.solved):,}")
            
            # Crystallize what we have as a region
            print(f"\n  Crystallizing checkpoint as region...")
            region = solver.crystallize_region(
                solver.current_region_name, 
                8  # Assume 8-piece
            )
            solver.total_solved += len(region.solved_hashes)
            print(f"  Region '{region.name}' saved with {len(region.solved_hashes):,} positions!")
    ...
    for region_idx in range(args.regions):
        ...
        if region_name in solver.regions:
            print(f"\nRegion '{region_name}' already solved ({len(solver.regions[region_name].solved_hashes):,} positions), skipping...")
            continue
    ...
    print("\nRegions:")
    for name, region in sorted(solver.regions.items()):
        print(f"  {name}: {len(region.solved_hashes):,} positions ({region.size_mb():.1f} MB)")
    
    # List checkpoints
    print("\nCheckpoints:")
    for f in sorted(os.listdir(solver.save_dir)):
        if 'checkpoint' in f:
            path = os.path.join(solver.save_dir, f)
