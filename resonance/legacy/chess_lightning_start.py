...
class SyzygyProbe:
    ...
    def probe(self, state):
        ...
        try:
            ...
            # Our Piece enum: KING=1, QUEEN=2, ROOK=3, BISHOP=4, KNIGHT=5, PAWN=6
            # python-chess: PAWN=1, KNIGHT=2, BISHOP=3, ROOK=4, QUEEN=5, KING=6
            our_to_chess = {
                1: chess.KING,    # W_KING/B_KING -> KING
                2: chess.QUEEN,   # W_QUEEN/B_QUEEN -> QUEEN
                3: chess.ROOK,    # W_ROOK/B_ROOK -> ROOK
                4: chess.BISHOP,  # W_BISHOP/B_BISHOP -> BISHOP
                5: chess.KNIGHT,  # W_KNIGHT/B_KNIGHT -> KNIGHT
                6: chess.PAWN,    # W_PAWN/B_PAWN -> PAWN
            }
            
            for p, sq in state.pieces:
                our_type = ((p - 1) % 6) + 1  # 1-6 regardless of color
                chess_type = our_to_chess[our_type]
                color = is_white(p)
                board.set_piece_at(sq, chess.Piece(chess_type, color))
...
class LightningSolver:
                    ...
                    if child_pieces <= 7:
                        ...
                        if value is not None:
                            ...
                            if self.metrics['contact_time'] is None:
                                ...
                                print(f"\n  ⚡ FIRST CONTACT! Lightning strikes at {child_pieces}-piece boundary")
    ...
    def solve(self, max_depth=50, positions_per_layer=50000):
        ...
        for depth in range(max_depth):
            ...
            solved_starts = sum(1 for h in list(self.all_seen)[:100] if h in self.solved)
            if solved_starts > 0:
                print(f"\n  🎉 {solved_starts} starting positions solved!")
...
def main():
    ...
    print("\n" + "="*60)
    print("⚡ 8-PIECE CHESS ENDGAME SOLVER - OVERNIGHT RUN ⚡")
    print("KQRRvKQRR - Lightning from canonical start")
    print("="*60)
    
    # THE canonical 8-piece starting position
    # White: Ke1, Qd1, Ra1, Rh1 (standard back rank)
    # Black: Ke8, Qd8, Ra8, Rh8 (standard back rank)
    start_pieces = [
        (Piece.W_KING, 4),    # e1
        (Piece.W_QUEEN, 3),   # d1
        (Piece.W_ROOK, 0),    # a1
    ...
    canonical_start = ChessState(start_pieces, 'w')
    print("\n★ CANONICAL 8-PIECE STARTING POSITION ★")
    canonical_start.display()
    
    solver.add_starting_position(canonical_start)
    canonical_hash = hash(canonical_start)
    
    # Add lots of random positions for exploration breadth
    print("\nGenerating 999 random positions for exploration breadth...")
    for i in range(999):
        state = solver.generate_random_start("KQRRvKQRR")
        if state:
            h = hash(state)
            if h not in solver.all_seen:
                solver.all_seen.add(h)
                solver.frontier[h] = state
                solver.state_turns[h] = state.turn
        if (i+1) % 200 == 0:
            print(f"  Generated {i+1}...")
    
    print(f"\nStarting with {len(solver.frontier)} positions")
    print("Running overnight - will report if canonical position gets solved!\n")
    
    # Solve! Let it run overnight with big batches
    solver.solve(max_depth=200, positions_per_layer=1000000)
    
    # Check if canonical start was solved
    if canonical_hash in solver.solved:
        result = solver.solved[canonical_hash]
        outcome = {1: "WHITE WINS", 0: "DRAW", -1: "BLACK WINS"}[result]
        print(f"\n{'='*60}")
        print(f"★★★ CANONICAL POSITION SOLVED: {outcome} ★★★")
        print(f"{'='*60}")
